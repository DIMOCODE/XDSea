...
contract XDSeaTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    event Create(address indexed _to, uint256 indexed tokenId, string status);
    ...
    address public xdsea721;
    address public xdsea1155;
    uint256 deci;
    uint256 public _tid;

    function getApproveStatus(
        address owneraddrrss,
        address contractaddress
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            ...
            if (
                IERC1155Upgradeable(_conAddr).balanceOf(from, ids[0]).sub(
                    ids[2]
                ) == 0
            ) {
                if (order_place[from][ids[0]].price > 0) {
    ...
    function _saleToken(
        ...
        address create;
        if (xdsea721 == _conAddr || xdsea1155 == _conAddr) {
            if (ids[3] == 721) {
                (create, ldatas[0]) = XDSea721(xdsea721).getCreatorsAndRoyalty(
                    ids[0]
                );
            } else {
                (create, ldatas[0]) = XDSea1155(xdsea1155)
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            XDSea721(xdsea721).mint(ipfsmetadata, msg.sender, datas[2], id_);
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], xdsea721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            XDSea1155(xdsea1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], xdsea1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(
        ...
        xdsea721 = _xdsea721;
        xdsea1155 = _xdsea1155;
    }

    function getTokenID(uint256 count) external returns (uint256) {
        require(
            msg.sender == xdsea721 || msg.sender == xdsea1155,
            "Permission denied"
        );
        _tid = _tid.add(count);
        return _tid;
    ...
    //_ids - tokenId, from - Owners, fromContract - contract Address
    function bulkBuy(
        uint256[] memory _ids,
        address[] memory from,
        ...
        require(_ids.length == from.length, "NFT length Mismatch");
        for (uint i = 0; i < _ids.length; i++) {
            require(
                amount[i] ==
                    order_place[from[i]][_ids[i]].price.mul(count[i]) &&
                    order_place[from[i]][_ids[i]].price.mul(count[i]) > 0,
                "Order Mismatch"
            );
            uint256[] memory tdatas = new uint256[](4);
            tdatas[0] = _ids[i];
            tdatas[1] = amount[i];
            tdatas[2] = count[i];
            tdatas[3] = _type[i];
            if (
                keccak256(abi.encodePacked((bidtoken[i]))) ==
                keccak256(abi.encodePacked(("coin")))
            ) {
                _saleToken(payable(from[i]), tdatas, "ETH",fromContract[i]);
            } else {
                _saleToken(payable(from[i]), tdatas, bidtoken[i],fromContract[i]);
            }
            if (_type[i] == 721) {
                IERC721Upgradeable(fromContract[i]).safeTransferFrom(
                    from[i],
                    msg.sender,
                    _ids[i]
                );
                if (order_place[from[i]][_ids[i]].price > 0) {
                    delete order_place[from[i]][_ids[i]];
                }
            } else {
                if (
                    IERC1155Upgradeable(fromContract[i]).balanceOf(from[i], _ids[i]).sub(
                        count[i]
                    ) == 0
                ) {
                    if (order_place[from[i]][_ids[i]].price > 0) {
                ...
                IERC1155Upgradeable(fromContract[i]).safeTransferFrom(
                    from[i],
                    msg.sender,
                    _ids[i],
                    count[i],
