    ...
    <NFTPage>
      ...
      {purchased ? (
        <TxModal
          ...
          confirmBtnPurchaise={() =>
            NavigateTo(`UserProfile/${isXdc(wallet?.address) ? fromXdc(wallet?.address) : wallet?.address}`)
          }
        ></TxModal>
      ) : null}
      <ContentNftPage>
          ...
          <HStack height="100%" responsive={true} alignment="flex-start">
            <VStack width="100%" padding="30px 15px">
              <VStack width={size.width < 768 ? "100%" : "540px"}>
                {(isXdc(wallet?.address) ? fromXdc(wallet?.address) : wallet?.address) === nft?.owner ? (
                  <LockedContent>
                    <VStack
                  ...
                  >
                    ...
                    {nft?.owner ? (
                      <Tooltip title={nft?.owner ? nft.owner : "-"}>
                        <BodyBold cursor={"pointer"}>{truncateAddress(nft?.owner)}</BodyBold>
                      </Tooltip>
                    ) : (
                      <ImpulseSpinner
                        size={30}
            ...
            <VStack width="100%" padding="30px 15px">
              <VStack width={size.width < 768 ? "100%" : "502px"}>
                    ...
                    <HStack spacing="9px" justify="flex-start">
                      ...
                      >
                        ...
                        <Tooltip title={nft?.creator ? nft.creator : "-"}>
                          <BodyBold cursor={"pointer"}>{truncateAddress(nft?.creator)}</BodyBold>
                        </Tooltip>
                      </HStack>
                      <CaptionRegular>
                      ...
                      {highestOffer === 0 ? (
                        ...
                        <>
                          <IconImg
                            ...
                            height="18px"
                          ></IconImg>
                          <TitleBold18>{parseInt(highestOffer).toLocaleString(undefined, {
                          maximumFractionDigits: 2,
                        })}</TitleBold18>
                          <CaptionBoldShort>XDC</CaptionBoldShort>
                        </>
                ...
                <HStack>
                  {wallet?.connected ? (
                    nft?.isListed ? (
                      nft?.owner === (isXdc(wallet?.address) ? fromXdc(wallet?.address) : wallet?.address) ? (
                        <>
                          <ButtonApp
                        ...
                        nft?.tokenId
                      ) ? null : nft?.owner === (isXdc(wallet?.address) ? fromXdc(wallet?.address) : wallet?.address) ? (
                      <>
                        <ButtonApp
                          icon={tagWhite}
