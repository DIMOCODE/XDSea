...
contract XDSea721Staking is ReentrancyGuard, IERC721Receiver, IERC1155Receiver {
    ...
    function saveAllRewards(
        ...
        for(uint i = 0; i < stakedNFTList.length; i++) {

            uint256 lastClaim = timeOfLastClaims[stakedNFTList[i]][_tokenContract];

            if(lastClaim <= stakedNFTs[stakedNFTList[i]].timeOfStake) {
                timeOfLastClaims[stakedNFTList[i]][_tokenContract] = stakedNFTs[stakedNFTList[i]].timeOfStake;
                lastClaim = stakedNFTs[stakedNFTList[i]].timeOfStake;
            }
            ...
            if(isBackedValue) {
                pendingRewards[stakedNFTList[i]][_tokenContract] += SafeMath.div(stakeCycles * (rewards[_tokenContract].rewardRate * stakedNFTs[stakedNFTList[i]].backedValue), 10000);
            }
            else {
                pendingRewards[stakedNFTList[i]][_tokenContract] += SafeMath.div(stakeCycles * rewards[_tokenContract].rewardRate, 10000);
            }

            // Update the time of the last claim for the staked NFT
            timeOfLastClaims[stakedNFTList[i]][_tokenContract] += SafeMath.mul(stakeCycles, rewards[_tokenContract].rewardFrequency);
        }
    }
    ...
    function saveRewardsForTokenID(
        ...
        if(lastClaim <= stakedNFTs[_tokenId].timeOfStake) {
            timeOfLastClaims[_tokenId][_tokenContract] = stakedNFTs[_tokenId].timeOfStake;
            lastClaim = stakedNFTs[_tokenId].timeOfStake;
        }
    ...
    function calculateRewards(
        ...
        for(uint i = 0; i < _rewards.length; i++) {
            ...
            if(stakedNFTs[_tokenId].isEligible) {
                ...
                if(timeOfLastClaims[_tokenId][_rewards[i]] <= stakedNFTs[_tokenId].timeOfStake) {
                    stakeDuration = block.timestamp - stakedNFTs[_tokenId].timeOfStake;
                }
    ...
    function _removeFromList(uint256 _tokenId) internal {
        for(uint i = 0; i < stakedNFTList.length; i++) {
            if(stakedNFTList[i] == _tokenId) {
                stakedNFTList[i] = stakedNFTList[stakedNFTList.length - 1];
                stakedNFTList.pop();
            }
        }
    }
    ...
    function claimRewards(
        ...
        if(timeOfLastClaims[_tokenId][_tokenContract] <= stakedNFTs[_tokenId].timeOfStake) {
            timeOfLastClaims[_tokenId][_tokenContract] = stakedNFTs[_tokenId].timeOfStake;
            claimDuration = block.timestamp - stakedNFTs[_tokenId].timeOfStake;
        }
